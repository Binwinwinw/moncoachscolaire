<?php
$page_title = 'Guide de remédiation 3ème - MonCoachScolaire';
$page_css = null;
...
$user_level = $_SESSION['user_level'] ?? '3ème';
...
<main class="main-content app-bg bg-gray-50 min-h-screen">
    ...
    <div class="header text-center py-8">
        <h1 class="text-3xl md:text-4xl font-bold <?php echo $theme['title']; ?> mb-2">📚 Guide de remédiation 3ème</h1>
        <p class="text-lg md:text-xl <?php echo $theme['subtitle']; ?> mb-6 font-medium">Programmes 2025 – Accompagnement personnalisé pour réussir ta 3ème et le Brevet</p>

        <!-- Boutons de navigation -->
        <div class="guide-navigation flex flex-wrap justify-center gap-4 mt-6">
            <a href="<?php echo site_url('college/3eme/exercices-3eme'); ?>" class="nav-btn nav-exercices <?php echo $theme['nav_primary']; ?> text-white px-6 py-3 rounded-lg font-semibold shadow transition focus:outline-none focus:ring-2" aria-label="Mes exercices 3eme">📝 Mes Exercices</a>
    ...
    <section class="content-section max-w-3xl mx-auto px-2 md:px-0">
        <div class="guide-content flex flex-col items-center gap-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 justify-center mb-8 w-full max-w-2xl mx-auto">
                ...
                <button type="button" onclick="openMatiereModal('brevet')" class="px-6 py-3 rounded-xl font-semibold shadow transition flex items-center gap-2 justify-center focus:outline-none focus:ring-2 <?php echo $subject_tones[0]; ?>">
                    <span class="text-2xl">📝</span> Prépa Brevet
                </button>
            </div>
        </div>
