    ...
    (function() {
        ...
        function setSaveButtonState(state, courseUrl = '') {
            ...
            if (state === 'saved') {
                ...
                btnSave.innerHTML = '📖 Voir ma révision';
            ...
            btnSave.disabled = false;
            btnSave.innerHTML = '📂 Sauvegarder dans mes révisions';
        ...
        function resolveSavedCourseUrl(saveData) {
            ...
            const candidateUrls = [];
            if (typeof saveData.revision_url === 'string' && saveData.revision_url.trim() !== '') {
                candidateUrls.push(saveData.revision_url.trim());
            }
            if (saveData.data && typeof saveData.data.revision_url === 'string' && saveData.data.revision_url.trim() !== '') {
                candidateUrls.push(saveData.data.revision_url.trim());
            }
            ...
            const revisionId = saveData.revision_id || (saveData.data && saveData.data.revision_id);
            if (revisionId) {
                return `${baseUrl}/index.php?page=revisions&id=${encodeURIComponent(revisionId)}`.replace(/\/\/+/g, '/');
            }
                ...
                try {
                    ...
                    if (data && previewHtml && data.cours) {
                        ...
                        const errorMessage = typeof data.error === 'string' ?
                            data.error :
                            (data.error.message || 'Erreur de génération');
                        quizIaResult.textContent = 'Erreur : ' + errorMessage;
                        quizIaResult.classList.add('text-red-600', 'py-4');
                    } else {
