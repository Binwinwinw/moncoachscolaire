...
<main class="main-content max-w-7xl mx-auto px-4 py-8">
        ...
        <div id="modal-quiz-ia-panel" class="modal-cours-ia-panel bg-white rounded-2xl shadow-2xl w-full relative flex flex-col" role="dialog" aria-labelledby="modal-cours-ia-title" aria-modal="true">
            <header class="modal-cours-ia-header flex items-start justify-between gap-4 px-6 pt-6 pb-4 border-b border-slate-100 shrink-0">
                <div>
                    ...
                    <p id="modal-cours-ia-subtitle" class="text-sm text-slate-500 mt-1">Choisis le niveau, la matière et un thème optionnel.</p>
                </div>
                <button type="button" id="close-modal-quiz-ia" class="modal-cours-ia-close text-gray-400 hover:text-gray-700 text-3xl leading-none shrink-0" aria-label="Fermer">&times;</button>
            </header>
    ...
    <div class="text-center mb-8">
        <h1 class="text-4xl md:text-5xl font-bold text-slate-800 mb-4 flex items-center justify-center gap-3">📚 Cours Interactifs</h1>
        <p class="text-xl text-slate-600 mb-6">Étudie avec des cours adaptés à ton niveau !</p>
...
if (empty($niveau_normalise)) :
    ...
    <?php else: ?>
    <script>window.COURS_USER_LEVEL = '<?php echo htmlspecialchars($niveau_normalise); ?>';</script>
    <?php endif; ?>
    ...
    <section class="my-12" id="random-cours-section">
        <div class="text-center mb-6">
            <h2 class="text-xl font-bold text-blue-700 mb-2">🎯 Ton cours à étudier</h2>
...
(function() {
    ...
    if (btnSave) {
        btnSave.addEventListener('click', async function() {
            ...
            try {
                ...
                if (saveData.success) {
                    setSaveButtonState('saved', saveData.course_url || '');
    ...
    if (formQuizIa) {
        formQuizIa.addEventListener('submit', async function(e) {
            ...
            if (!niveau && window.COURS_USER_LEVEL) {
                niveau = window.COURS_USER_LEVEL.toLowerCase().replace('é', 'e').replace('è', 'e');
            }

            if (!niveau || !matiere) {
                quizIaResult.innerHTML = '<div class="text-red-600 py-4">Merci de choisir un niveau et une matière.</div>';
                return;
            }
