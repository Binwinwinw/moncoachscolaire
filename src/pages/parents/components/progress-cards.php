...
<section>
    ...
    <?php if (empty($enfants)): ?>
        <div class="rounded-2xl p-8 text-center text-gray-500 shadow">Aucun enfant rattaché à ce compte pour l’instant.<br>Ajoutez un enfant pour suivre sa progression !</div>
    <?php else: ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <?php foreach (is_array($progressions) ? $progressions : [] as $prog): ?>
    ...
    <ul class="mb-8">
        <?php foreach (is_array($dernierExos) ? $dernierExos : [] as $exo): ?>
    ...
    <div class="flex flex-wrap gap-2">
        <?php foreach (is_array($matieresFortes) ? $matieresFortes : [] as $m): ?>
            <span class="px-3 py-1 rounded-full text-xs font-semibold <?= $m['forte'] ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700' ?>">
                <?= htmlspecialchars($m['nom']) ?> <?= $m['forte'] ? '✓' : '⚠️' ?>
            </span>
        <?php endforeach; ?>
    </div>
</section>
