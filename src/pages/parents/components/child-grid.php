    ...
    <?php if (empty($enfants)): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            $demoEnfants = [
                [
                    'prenom' => 'Emma',
                    'niveau' => '6e',
                    'avatar' => '/assets/img/avatar-demo-6e.png',
                    'etat' => 'Démo',
                ],
                [
                    'prenom' => 'Lucas',
                    'niveau' => '3e',
                    'avatar' => '/assets/img/avatar-demo-3e.png',
                    'etat' => 'Démo',
                ],
            ];
        foreach ($demoEnfants as $enfant): ?>
                <div class="rounded-2xl shadow p-6 flex flex-col items-center hover:scale-[1.02] transition-transform">
                    <img src="<?= htmlspecialchars($enfant['avatar']) ?>" alt="Avatar" class="w-16 h-16 rounded-full mb-2 border-2 border-indigo-100">
    ...
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php foreach ($enfants as $enfant): ?>
                <div class="rounded-2xl shadow p-6 flex flex-col items-center hover:scale-[1.02] transition-transform">
                    <img src="<?= htmlspecialchars($enfant['avatar'] ?? '/assets/img/avatar-default.png') ?>" alt="Avatar" class="w-16 h-16 rounded-full mb-2 border-2 border-indigo-100">
