<?php
// Exporte les exercices différents entre all_exercises.json et all_exercises_clean.json
// Usage : php dev/tools/export_exercises_diff_json.php
...
function normalize($ex) {
    ...
    return [
        'Subject' => strtolower(trim($ex['Subject'] ?? '')),
        'Level' => strtolower(trim($ex['Level'] ?? '')),
        'Title' => strtolower(trim(strip_tags($ex['Title'] ?? ''))),
        'Content' => strtolower(trim(strip_tags($ex['Content'] ?? ''))),
    ];
}
...
echo "Export terminé : " . count($diff) . " exercices différents exportés dans $outputFile\n";
