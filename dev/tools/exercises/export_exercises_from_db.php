 ...
 * =============================================================
 *  Ce script extrait TOUS les exercices de la table exercises,
 *  les exporte au format JSON (même structure que les .migrated.json),
 *  et génère un rapport d'extraction détaillé.
 * =============================================================
 */
...
try {
    ...
    die("❌ Erreur SQL lors de l'extraction : {$e->getMessage()}\n\n");
}
...
foreach ($exercises as &$exercise) {
    ...
    $exercise['xp_points'] = (int) $exercise['xp_points'];
    $exercise['is_active'] = (int) $exercise['is_active'];
    ...
    $answerType = $exercise['answer_type'] ?? 'unknown';
...
if (!empty($stats['byDifficulty'])) {
    ...
    $difficultyOrder = ['facile' => 0, 'moyen' => 0, 'difficile' => 0];
    foreach ($stats['byDifficulty'] as $diff => $count) {
        if (isset($difficultyOrder[$diff])) {
            $difficultyOrder[$diff] = $count;
        } else {
            echo sprintf("  • %-20s : %4d exercices\n", $diff, $count);
        }
    }
    foreach ($difficultyOrder as $diff => $count) {
        if ($count > 0) {
            echo sprintf("  • %-20s : %4d exercices\n", $diff, $count);
        }
    }
