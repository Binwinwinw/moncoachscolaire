<?php
/**
 * IMPORT DES EXERCICES NETTOYÉS (STAGE 2)
 *
 * Importe le fichier exercises_stage2_cleaned.json dans la base de données
 *
 * Modes :
 * - --dry-run : Simulation sans modification de la BDD
 * - --truncate : Vide la table avant l'import (DANGEREUX)
 * - (défaut) : Mode UPDATE (met à jour les existants, insère les nouveaux)
 *
 * Usage :
 *   php import_cleaned_exercises.php --dry-run
 *   php import_cleaned_exercises.php
 *   php import_cleaned_exercises.php --truncate
 */
...
try {
    // Récupère la connexion PDO depuis connection.php
    global $pdo;
...
// 3. TRUNCATE (si demandé)
if ($truncate && !$dryRun) {
    echo "⚠️  MODE TRUNCATE : Vidage de la table...\n";
...
foreach ($exercises as $index => $ex) {
    ...
    try {
        ...
        // Garder les 50 premières actions
        if (count($actions) < 50) {
            $actions[] = [
                'identifier' => $identifier,
                'action' => $action,
...
$txt .= "Mode : " . ($dryRun ? 'DRY-RUN (simulation)' : ($truncate ? 'TRUNCATE' : 'UPDATE')) . "\n";
...
if ($dryRun) {
    ...
    echo "   • " . REPORT_DIR . "/import_cleaned_report.txt\n";
    echo "   • " . REPORT_DIR . "/import_cleaned_report.json\n\n";

    if ($stats['null_answers'] > 0) {
        echo "⚠️  ATTENTION : " . $stats['null_answers'] . " exercices n'ont pas de réponse.\n";
        echo "   Pour les désactiver temporairement :\n";
