<?php
// Script unique pour l'audit, la correction et l'enrichissement des exercices
// Usage : php dev/tools/exercises_data_cleaner.php

$sourceFile = __DIR__ . '/../../db/json/all_exercises_clean.json';
$outputFile = __DIR__ . '/../../db/json/all_exercises_clean_enriched.json';
...
function get_difficulty($ex) {
    // Simple heuristique : longueur, présence de calculs, choix, etc.
    $content = strtolower($ex['Content'] ?? '');
    $answer = strtolower($ex['Answer'] ?? '');
    $choices = $ex['Choices'] ?? null;
    if (strpos($content, 'démontrer') !== false || strpos($content, 'justifie') !== false || strpos($content, 'calculer') !== false) return 'difficile';
...
function normalize_instruction($ex) {
    // Si Content et Instruction sont identiques, on tente d'enrichir Instruction
