...
echo "\n";
echo "═══════════════════════════════════════════════════════════════\n";
echo "  DÉTECTION & DÉDOUBLONNAGE D'EXERCICES (+ Normalisation)\n";
echo "═══════════════════════════════════════════════════════════════\n\n";
...
echo "🔧 Normalisation des exercices (garantie 27 champs BDD)...\n";

$dbExercises = ExerciseNormalizer::normalizeAllToDbFields($dbExercises);
$jsonExercises = ExerciseNormalizer::normalizeAllToDbFields($jsonExercises);

echo "✅ Tous les exercices sont normalisés (champs BDD)\n";
echo "───────────────────────────────────────────────────────────────\n\n";
...
// ─────────────────────────────────────────────────────────────
// 💾 CRÉATION DU FICHIER DÉDOUBLONNÉ (+ NORMALISATION)
// ─────────────────────────────────────────────────────────────
...
// 4. ✅ Normaliser TOUS les exercices finaux

// ───── Ajouter les exercices JSON (priorité) ─────
foreach ($jsonExercises as $ex) {
    $id = $ex['identifier'] ?? null;
    if ($id && !isset($processedIdentifiers[$id])) {
        // ✅ Garantie : déjà normalisé ci-dessus
        $cleanExercises[] = $ex;
...
// ✅ VÉRIFICATION FINALE : tous les exercices sont normalisés (champs BDD)
$conformityReport = ExerciseNormalizer::getConformityReportDb($cleanExercises);
...
$logLines = [];
$logLines[] = "═══════════════════════════════════════════════════════════════";
$logLines[] = "  RAPPORT DE DÉDOUBLONNAGE (+ NORMALISATION) — " . date('Y-m-d H:i:s');
$logLines[] = "═══════════════════════════════════════════════════════════════\n";
...
if ($conformityReport['invalid'] > 0) {
    $logLines[] = "═══════════════════════════════════════════════════════════════";
    $logLines[] = "EXERCICES INVALIDES (champs obligatoires manquants)";
    $logLines[] = "═══════════════════════════════════════════════════════════════";
    foreach ($conformityReport['invalid_exercises'] as $inv) {
        $logLines[] = "[{$inv['index']}] {$inv['identifier']} — Champs manquants : " . implode(', ', $inv['missing']);
    }
    $logLines[] = "";
}
