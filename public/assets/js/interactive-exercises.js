    ...
    function detectExerciseType(content, instruction) {
        ...
        if (/trace(r)?|placer|droite|triangle|figure|segment|cercle|géométrie/.test(txt)) return 'graph';
        if (/glisse|associe|relie|déplace|drag|drop|classe|range|ordre/.test(txt)) return 'dragdrop';
        if (/tableau|remplis|complète le tableau|case|cellule/.test(txt)) return 'table';
        if (/code|programme|algorithme|python|scratch|complète le code/.test(txt)) return 'code';
        if (/simul|manipul|expérience|variable|observe|expérimente/.test(txt)) return 'simulation';
        if (/chronolog|ligne du temps|timeline|événement|classement avancé/.test(txt)) return 'advanced-chronology';
        if (/qcm|choix multiple|coche|sélectionne/.test(txt)) return 'qcm';
        if (/texte à trou|complète le texte|saisie libre|rédige|écris/.test(txt)) return 'texte';
        // Fallback
        return 'texte';
    }
...
(function() {
    ...
    function initWordColoringExercise() {
        const coloringExercises = document.querySelectorAll('.word-coloring-exercise');
        ...
        coloringExercises.forEach(exercise => {
            ...
            const container = exercise.querySelector('.word-coloring-container');
            ...
            words.forEach((word, index) => {
                ...
                wordSpan.addEventListener('click', function() {
                    ...
                    // Appliquer le style
                    this.classList.remove('colored-blue', 'colored-green', 'colored-red');
                    if (nextColor !== 'none') {
                        this.classList.add('colored-' + nextColor);
                    }

                    this.dataset.wordType = nextColor;
                    this.title = 'Clique pour changer : ' + (colorNames[nextColor] || 'aucune couleur');
                });
            ...
            if (checkBtn && feedbackDiv) {
                checkBtn.addEventListener('click', function() {
                    ...
                    container.querySelectorAll('.coloring-word').forEach(wordSpan => {
                        const word = wordSpan.dataset.word;
                        const userType = wordSpan.dataset.wordType || 'none';
                        ...
                        if (correctType && correctType !== 'determinant') {
                            ...
                            const isCorrect =
                                (userType === 'blue' && correctType === 'nom') ||
                                (userType === 'green' && correctType === 'verbe') ||
                                (userType === 'red' && correctType === 'adjectif');

                            wordSpan.classList.remove('correct-answer', 'wrong-answer');
    ...
    function initConjugationExercise() {
        const conjugationExercises = document.querySelectorAll('.conjugation-exercise');

        console.log('🔄 Initialisation des exercices de conjugaison:', conjugationExercises.length);
        ...
        conjugationExercises.forEach((exercise, exerciseIndex) => {
            ...
            try {
                ...
                console.log(`  ✅ Exercice ${exerciseIndex + 1}: ${questions.length} question(s) trouvée(s)`, questions);
            } catch (e) {
                console.error('❌ Erreur parsing questions:', e, exercise.dataset.questions);
            }
            ...
            if (!container) {
                console.warn('⚠️ Container .conjugation-container non trouvé pour l\'exercice', exerciseIndex + 1);
                return;
            }

            console.log(`  📝 Création des champs pour l'exercice ${exerciseIndex + 1}...`);

            // Vider le conteneur
            container.innerHTML = '';

            if (questions.length === 0) {
                console.warn(`  ⚠️ Aucune question trouvée pour l'exercice ${exerciseIndex + 1}`);
                container.innerHTML = '<p style="color: #ef4444; padding: 1rem;">Aucune question disponible pour cet exercice.</p>';
                return;
            }
            ...
            questions.forEach((q, index) => {
                ...
                const answer = q.answer || (q.answers && q.answers.length > 0 ? q.answers[0] : '');
                ...
                if (sentence.includes('(_0_)') || (answers.length > 0 && adjectives.length > 0)) {
                    // Remplacer chaque placeholder par un input
                    let currentSentence = sentence;
                    const inputStyle = "display: inline-block; min-width: 120px; margin: 0 0.3rem; padding: 0.5rem; border: 2px solid #3b82f6; border-radius: 4px;";

                    // Remplacer tous les placeholders _X_ par des inputs
                    for (let i = 0; i < Math.max(answers.length, adjectives.length); i++) {
                        ...
                        if (currentSentence.includes(placeholder)) {
                            ...
                            const inputHTML = `<input type="text"
                                ...
                                autocomplete="off"
                                style="${inputStyle}">`;
                            currentSentence = currentSentence.replace(placeholderRegex, inputHTML);
                        }
                    }
                ...
                questionDiv.innerHTML = displayHTML + '<span class="input-feedback"></span>';
            ...
            if (checkBtn && feedbackDiv) {
                checkBtn.addEventListener('click', function() {
                    let correct = 0;
                    const inputs = container.querySelectorAll('.conjugation-input');
                    ...
                    inputs.forEach(input => {
                        ...
                        if (!feedbackSpan || !feedbackSpan.classList.contains('input-feedback')) {
                            // Chercher dans le parent
                            const questionDiv = input.closest('.conjugation-question');
                            if (questionDiv) {
                                feedbackSpan = questionDiv.querySelector('.input-feedback');
                            }
                        }
                        ...
                        if (!userAnswer) {
                            if (feedbackSpan) {
                                feedbackSpan.textContent = '⚠️ Tu n\'as pas encore répondu';
                    ...
                    const percentage = inputs.length > 0 ? Math.round((correct / inputs.length) * 100) : 0;
                    let feedbackHTML = '';
        ...
        mathExercises.forEach(exercise => {
            ...
            if (checkBtn && feedbackDiv) {
                checkBtn.addEventListener('click', function() {
                    ...
                    if (crystalsAwarded > 0 && typeof awardCrystals === 'function') {
                        awardCrystals(crystalsAwarded);
                    }

                    // Sauvegarder la progression
                    const exerciseCard = exercise.closest('.exercise-card');
                    const exerciseId = exerciseCard ? (exerciseCard.dataset.exerciseId || null) : null;
                    const difficulty = exerciseCard ? (exerciseCard.dataset.difficulty || 'moyen') : 'moyen';

                    if (exerciseId && typeof saveProgressToServer === 'function') {
                        const rewards = calculateRewards(percentage, difficulty);
    ...
    function initQCMExercise() {
        ...
        qcmExercises.forEach(exercise => {
            ...
            if (questions.length === 0) {
                console.warn('⚠️ Aucune question dans le tableau pour cet exercice QCM');
                container.innerHTML = '<p style="color: #ef4444;">Aucune question disponible pour cet exercice.</p>';
                return;
            }

            console.log('  ✅ Création de ' + questions.length + ' question(s) QCM');
            ...
            if (checkBtn && feedbackDiv) {
                checkBtn.addEventListener('click', function() {
                    let correct = 0;
                    const questionDivs = container.querySelectorAll('.qcm-question');

                    questionDivs.forEach((questionDiv, index) => {
                        const selectedRadio = questionDiv.querySelector('input[type="radio"]:checked');
                        const feedbackSpan = questionDiv.querySelector('.input-feedback');
                        const correctRadio = questionDiv.querySelector('input[data-correct="true"]');

                        questionDiv.querySelectorAll('.qcm-choice').forEach(choice => {
                        ...
                        if (isCorrect) {
                            correct++;
                            selectedRadio.closest('.qcm-choice').classList.add('correct-choice');
                            feedbackSpan.textContent = '✅ Correct !';
                            feedbackSpan.className = 'input-feedback success';
                            feedbackSpan.style.display = 'inline-block';
                        } else {
                            selectedRadio.closest('.qcm-choice').classList.add('wrong-choice');
                            if (correctRadio) {
                                correctRadio.closest('.qcm-choice').classList.add('correct-choice');
                            }
                            const correctLabel = correctRadio ? correctRadio.closest('.qcm-choice').querySelector('.qcm-choice-text').textContent : '';
                    ...
                    // Afficher le feedback global
                    const percentage = questionDivs.length > 0 ? Math.round((correct / questionDivs.length) * 100) : 0;
                    let feedbackHTML = '';
                    ...
                    const exerciseId = exerciseCard ? (exerciseCard.dataset.exerciseId || null) : null;
                    const difficulty = exerciseCard ? (exerciseCard.dataset.difficulty || 'moyen') : 'moyen';
                    const subject = exerciseCard ? (exerciseCard.dataset.subject || 'Français') : 'Français';

                    if (exerciseId) {
                        setTimeout(() => {
                            const saveFn = window.InteractiveExercises?.saveProgress ||
                                         (typeof saveProgressToServer === 'function' ? saveProgressToServer : null);
                            const calcFn = window.InteractiveExercises?.calculateRewards ||
                                         (typeof calculateRewards === 'function' ? calculateRewards : null);
    ...
    function initChronologyExercise() {
        const chronologyExercises = document.querySelectorAll('.chronology-exercise');
        ...
        chronologyExercises.forEach(exercise => {
            ...
            const selectsContainer = container.querySelector('.chronology-selects');
            const correctOrder = events.map((e, i) => i).sort((a, b) => {
            ...
            if (checkBtn && feedbackDiv) {
                checkBtn.addEventListener('click', function() {
                    let correct = 0;
                    const selects = container.querySelectorAll('.chronology-select');
                    const selectedValues = Array.from(selects).map(s => parseInt(s.value));
                    ...
                    selectedValues.forEach((selectedIndex, position) => {
                        if (selectedIndex === correctOrder[position]) {
                            ...
                            selects[position].classList.remove('correct-select');
                        }
                    });

                    // Afficher le feedback
                    const percentage = Math.round((correct / events.length) * 100);
                    let feedbackHTML = '';
    ...
    function initGraphExercise() {
        ...
        graphExercises.forEach(exercise => {
            try {
                ...
                ctx.beginPath();
                ctx.moveTo(40, 260); ctx.lineTo(360, 260); // axe x
                ctx.moveTo(40, 260); ctx.lineTo(40, 40);   // axe y
                ctx.stroke();

                // --- Consigne ---
                let instruction = exercise.dataset.instruction || 'Cliquez pour placer les points du triangle rectangle en B.';
                let instructionDiv = exercise.querySelector('.graph-instruction');
                if (!instructionDiv) {
                ...
                canvas.addEventListener('click', function(e) {
                    ...
                    if ((shape === 'triangle' && points.length === 3) || (shape === 'line' && points.length === 2)) {
                        ctx.beginPath();
                ...
                checkBtn.onclick = function() {
                    if ((shape === 'triangle' && points.length !== 3) || (shape === 'line' && points.length !== 2)) {
                        showGraphFeedback(exercise, '❌ Placez tous les points avant de vérifier.', 'error');
                        return;
                    }
                    ...
                    if (shape === 'triangle') {
                        ...
                        const isRight = Math.abs(Math.pow(AB,2) + Math.pow(BC,2) - Math.pow(CA,2)) < 10;
                        if (isRight) {
                            showGraphFeedback(exercise, '✅ Bravo ! Triangle rectangle en B.', 'success');
                        } else {
                            showGraphFeedback(exercise, '❌ Le triangle n’est pas rectangle en B.', 'error');
                        }
                    } else if (shape === 'line') {
                        showGraphFeedback(exercise, '✅ Droite tracée entre les deux points.', 'success');
                    }
                };
            } catch (err) {
                ...
                feedback.textContent = 'Erreur lors de l\'initialisation de l\'exercice graphique.';
    ...
    function initDragDropExercise() {
        const dragDropExercises = document.querySelectorAll('.dragdrop-exercise');
    ...
    function initSimulationExercise() {
        const simulationExercises = document.querySelectorAll('.simulation-exercise');
    ...
    function initAdvancedChronologyExercise() {
        const advChronologyExercises = document.querySelectorAll('.advanced-chronology-exercise');
        advChronologyExercises.forEach(exercise => {
            // TODO: Initialisation classement chronologique avancé (drag & drop, timeline)
            console.log('⬛ Initialisation exercice chronologie avancée:', exercise);
        });
    }
    ...
    function initAll() {
        ...
        console.log('  - QCM:', document.querySelectorAll('.qcm-exercise').length);
        console.log('  - Math:', document.querySelectorAll('.math-exercise').length);
        console.log('  - Conjugation:', document.querySelectorAll('.conjugation-exercise').length);
        console.log('  - Word Coloring:', document.querySelectorAll('.word-coloring-exercise').length);
        console.log('  - Chronology:', document.querySelectorAll('.chronology-exercise').length);
        console.log('  - Graphique:', document.querySelectorAll('.graph-exercise').length);
        console.log('  - Drag & Drop:', document.querySelectorAll('.dragdrop-exercise').length);
        console.log('  - Tableau:', document.querySelectorAll('.table-exercise').length);
        console.log('  - Code:', document.querySelectorAll('.code-exercise').length);
        console.log('  - Simulation:', document.querySelectorAll('.simulation-exercise').length);
        console.log('  - Chronologie avancée:', document.querySelectorAll('.advanced-chronology-exercise').length);
        ...
        document.querySelectorAll('.exercise-auto').forEach(exercise => {
            ...
            switch (detectedType) {
                case 'graph': initGraphExercise && initGraphExercise(); break;
                case 'dragdrop': initDragDropExercise && initDragDropExercise(); break;
                case 'table': initTableExercise && initTableExercise(); break;
                case 'code': initCodeExercise && initCodeExercise(); break;
                case 'simulation': initSimulationExercise && initSimulationExercise(); break;
                case 'advanced-chronology': initAdvancedChronologyExercise && initAdvancedChronologyExercise(); break;
                case 'qcm': initQCMExercise && initQCMExercise(); break;
                case 'texte': default: /* fallback */ break;
            }
        });
    ...
    async function saveProgressToServer(exerciseData) {
        ...
        try {
            ...
            if (result.fallback || !result.success) {
                ...
                console.log('Progression sauvegardée localement (BDD indisponible ou erreur)');
                ...
                document.dispatchEvent(new CustomEvent('exerciseCompleted', {
                    detail: {
                        ...
                        badge: result.badge_unlocked ? result.badge_name : null,
            ...
            console.error('Erreur sauvegarde, utilisation localStorage:', error);
            saveProgressToLocalStorage(exerciseData);
    ...
    function saveProgressToLocalStorage(data) {
        ...
        const pending = JSON.parse(localStorage.getItem('pending_progress_saves') || '[]');
        pending.push(key);
    ...
    async function syncLocalStorageToServer() {
        ...
        const pendingKeys = JSON.parse(localStorage.getItem('pending_progress_saves') || '[]');

        if (pendingKeys.length === 0) {
            return;
        }

        console.log(`Tentative de synchronisation de ${pendingKeys.length} sauvegardes...`);
        ...
        localStorage.setItem('pending_progress_saves', JSON.stringify(failedKeys));

        if (syncedKeys.length > 0) {
            console.log(`${syncedKeys.length} sauvegardes synchronisées avec succès`);
        }
